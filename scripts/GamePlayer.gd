...
### 		MeleeRayCast (cast_to: Vector3(0, 0, -2))

const LOWER_AIM_LIMIT:float = (-PI/2) + deg2rad(15)
const UPPER_AIM_LIMIT:float = (PI/2) - deg2rad(15)
...
func attack_melee_ray():
	if can_attack and $Camera/MeleeRayCast.is_colliding():
