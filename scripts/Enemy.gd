...
# Attach to a KinematicBody with animations "walk-loop" and "attack"
# Character walks in random directions, colliding off other physics objects.
# When a node in the group PLAYER_GROUP is near, target and attack them.
# distance_squared_to is harder to use than distance_to but it's faster (skips sqrt() calculation)
